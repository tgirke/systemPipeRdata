...
or with a single load command, and then run the workflow with a another single command. 

This `Generic` workflow template includes the following four data processing steps.
...
in the main vignette uses the same workflow steps to introduce the basic usage
of `systemPipeR`.

## Create workflow environment

The environment of the chosen `Generic` workflow is generated with the `genWorenvir` 
...
The `SPRproject` initializes a new workflow project instance. This function
...
sal
```
Workflow instances in `systemPipeR` can be constructed stepwise in interactive
mode by evaluating the code of individual workflow steps in the R console
one-by-one. Alternatively, one can import all steps of a workflow with a single
import command at once, either from an R script or an R Markdown workflow file.
The following introduces first the stepwise approach, and then the
automated import approach (see [here](#importwf)). 
...
__Note:__ To demonstrate the standard automation routines, the first line below uses the
`overwrite=TRUE` option of the `SPRproject` function. 
This option is generally discouraged as it erases the existing workflow project and `sal` container. 
It is used here for brevity. For information on resuming and restarting workflow runs, consult the 
relevant section of the main vignette (see [here](https://www.bioconductor.org/packages/devel/bioc/vignettes/systemPipeR/inst/doc/systemPipeR.html#10_Restarting_and_resetting_workflows).)

```{r , import_run_routine, eval=FALSE, spr=TRUE}
sal <- SPRproject(overwrite = TRUE) # Avoid 'overwrite=TRUE' in real runs.
sal <- importWF(sal, file_path = "new.Rmd") #
sal <- runWF(sal)
