...
the corresponding `HTML` report generated after running a workflow on the provided test
data. A list of the default data analysis steps included in each workflow is
given [here](#wf-template-steps). Similar and additional workflow templates are also 
available from GitHub (see [below](#wf-github-collection) and [here](https://systempipe.org/about/project/#core-packages)), 
which can be retrieved directly from their repositories.
To create an empty workflow template without
...
In addition to the templates distributed through `systemPipeRdata`, similar workflow
templates can also be retrieved directly from GitHub repositories using the
`genWorkenvir_gh` function. Details on this are provided [below](#wf-github-collection).
...
# Additional workflow templates on GitHub {#wf-github-collection}

In addition to the workflow templates distributed with the `systemPipeRdata`
package, similar templates are available as standalone repositories
on GitHub. These workflows include both well-established and experimental
analysis pipelines that extend the range of supported applications.
A curated list of available workflow repositories is maintained on the
systemPipe project website [here](https://systempipe.org/about/project/).
Each repository contains a complete workflow environment including parameter
files, example input data (when available), and documentation describing the
workflow structure and usage.

Workflow environments hosted on GitHub can be initialized directly from within
R using the `genWorkenvir_gh()` function. For example:

```{r genworkenvirgh, eval=FALSE}
genWorkenvir_gh(url = "https://github.com/systemPipeR/sprwf-rnaseq.git", mydirname = "sprwf-rnaseq")
```

This command downloads the workflow repository and creates a local workflow
environment that can be executed or customized by the user. After
initialization, the R session should be directed to the resulting project
directory, and the `param` and `data` directories loaded with the 
`getParam_gh` and `getData_gh` functions. Detailed instructions and documentation 
for each workflow are provided within the corresponding GitHub repositories.
