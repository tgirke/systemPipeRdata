...
$ Rscript -e "systemPipeRdata::genWorkenvir(workflow='SPrnaseq', mydirname='rnaseq')"
```
...
# Experiment templates collection

A collection of experimental templates are available, and it is possible to browse the 
...
and [systemPipeR Organization](https://github.com/systemPipeR) on GitHub. 

To the the experiment templates, use `git` or `git-bash.exe` on Windows
to clone the repository with the
`Download URL` provided in the table above. 
