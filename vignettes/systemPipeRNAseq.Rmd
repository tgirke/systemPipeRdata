...
`systemPipeRdata`, a companion package to systemPipeR [@H_Backman2016-bt].
...
The `Rmd` file associated with this vignette serves a dual purpose. It acts
...
can be loaded from the R Markdown (`Rmd`) file of this vignette, here named 
`systemPipeRNAseq.Rmd`. All data analysis steps of the workflow will be stored in 
a workflow management container called `SYSargsList` or short `SAL` (below `sal`). 
Additional details about the `SAL` container (S4 class) are available in the main vignette 
