...
\begin{figure}[H]
  \centering
   \includegraphics[width=10cm]{./results/sample_tree.pdf}
...
\begin{figure}[H]
  \centering
   \includegraphics[width=10cm]{./results/DEGcounts.pdf}
   \caption{Up and down regulated DEGs with FDR of 10\%.}
   \label{fig:DEGcounts}
\end{figure}
...
\begin{figure}[H]
  \centering
   \includegraphics[width=14cm]{./results/vennplot.pdf}
...
\begin{figure}[H]
  \centering
   \includegraphics[width=20cm]{./results/GOslimbarplotMF.pdf}
   \caption{GO Slim Barplot for MF Ontology.}
   \label{fig:GOMF}
\end{figure}

\section{Clustering and heat maps}
The following example performs hierarchical clustering on the \Rfunarg{rlog} normalized expression matrix subsetted by the DEGs identified in the above differential expression analysis. It uses a Pearson correlation-based distance measure and complete linkage for cluster joining.
<<eval=FALSE>>=
library(pheatmap)
...
\begin{figure}[H]
  \centering
   \includegraphics[width=12cm]{./results/heatmap1.pdf}
