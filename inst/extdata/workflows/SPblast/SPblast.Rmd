...
The below workflow steps can be imported from the source `Rmd` file of this
vignette all at once into a `SAL` (`SYSargsList`) workflow container using the
`importWF` function. After this the entire workflow can be run from start to
finish with the `runWF` function. Details on this are provided in the next
section [here](#importwf)
