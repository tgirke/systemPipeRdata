...
##     p <- GsnapParam(genome=gmapGenome, unique_only=TRUE, molecule="DNA",
##                     max_mismatches=3)
...
## args <- systemArgs(sysma="param/filter_sambcf.param",
##                    mytargets="targets_sambcf.txt")[1:4]
...
## args <- systemArgs(sysma="param/annotate_vars.param",
##                    mytargets="targets_sambcf_filtered.txt")
## txdb <- loadDb("./data/tair10.sqlite")
## fa <- FaFile(systemPipeR::reference(args))
## suppressAll(variantReport(args=args, txdb=txdb, fa=fa, organism="A. thaliana"))
...
## args <- systemArgs(sysma="param/annotate_vars.param",
##                    mytargets="targets_vartools_filtered.txt")
## txdb <- loadDb("./data/tair10.sqlite")
## fa <- FaFile(systemPipeR::reference(args))
## suppressAll(variantReport(args=args, txdb=txdb, fa=fa, organism="A. thaliana"))
...
## args <- systemArgs(sysma="param/annotate_vars.param",
##                    mytargets="targets_gatk_filtered.txt")
## varlist <- sapply(names(outpaths(args))[1:4],
...
##                       param=ScanBamParam(which=GRanges(mychr,
