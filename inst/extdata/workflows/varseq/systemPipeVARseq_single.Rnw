...
% R CMD Sweave --engine=knitr::knitr --pdf systemPipeVARseq_single.Rnw
...
\title{VAR-Seq Workflow Report for Single Computer}
\author{Project ID: VARseq\_PI\_Name\_Organism\_July2015 \\
...
\begin{figure}[H]
  \centering
   \includegraphics[width=10cm]{results/vennplot_var.pdf}
   \caption{Venn Diagram for 4 samples from GATK, BCFtools and VariantTools.}
   \label{fig:vennplot}
\end{figure}
